...
const FONT_OPTIONS = [
  {
    ...
    family: "'Segoe Script', 'Caveat', cursive"
  },
  {
    id: "hand",
...
const App = () => {
  ...
  const modifyTable = (action: string) => {
    ...
    if (action === "addRow") {
      ...
      alert("Column deletion in complex tables is restricted.");
      ...
      if (neighborX < grid[rIdx].length) {
        ...
        if (neighbor && neighbor !== cell) {
          if ((cell.rowSpan || 1) === (neighbor.rowSpan || 1)) {
            ...
            neighbor.remove();
          } else alert("Cannot merge cells of different heights.");
        }
      }
      ...
      if (neighborY < grid.length) {
        ...
        if (neighbor && neighbor !== cell) {
          if ((cell.colSpan || 1) === (neighbor.colSpan || 1)) {
            ...
            neighbor.remove();
          } else alert("Cannot merge cells of different widths.");
        }
      }
    }
    triggerParse();
  ...
  const parseContentToPages = (root: HTMLElement) => {
    ...
    const processTable = (tableEl: HTMLElement): TextSegment | null => {
      ...
      rows.forEach((tr, rIdx) => {
        ...
        cells.forEach(cell => {
          ...
          for (let r = 0; r < rs; r++) {
            for (let c = 0; c < cs; c++) {
              if (grid[rIdx + r])
                grid[rIdx + r][cIdx + c] = r === 0 && c === 0 ? el : null;
            }
          }
      ...
      cellMap.forEach((pos, el) => {
        ...
        const fontStr = `${isBold ? "bold" : "normal"} ${CURRENT_FONT_SIZE}px ${
          CURRENT_FONT.family
        }`;
        const w =
          measureTextWithSpacing(text, fontStr, spacingFactor) + 20 * SCALE;
      ...
      for (let r = 0; r < rows.length; r++) {
        ...
        for (let c = 0; c < maxCols; c++) {
          ...
          if (el) {
            ...
            if (!finalColor) {
              ...
              if (sChild)
                finalColor =
                  sChild.getAttribute("color") ||
                  (sChild as HTMLElement).style.color;
            }
            const isBold =
              el.tagName === "TH" ||
            ...
            const align = (
              ["center", "right"].includes(alignVal) ? alignVal : "left"
            ) as AlignType;
        ...
        tableRows.push({
          cells: rowData,
          maxHeight: CURRENT_LINE_HEIGHT * 1.2
        });
      }
      return {
        type: "table",
    ...
    const traverse = (
      node: Node,
      style: { color: string; isBold: boolean; isUnderline: boolean },
      listContext: { type: string; index: number } | null
    ) => {
      if (node.nodeType === Node.ELEMENT_NODE) {
        ...
        const isBlock = ["DIV", "P", "BR", "LI", "H1", "H2", "TR"].includes(
          tagName
        );
        ...
        let newColor =
          el.style.color || el.getAttribute("color") || style.color;
        const newBold =
          tagName === "B" ||
          tagName === "STRONG" ||
          tagName === "TH" ||
        ...
        if (tagName === "LI" && listContext) {
          const bullet =
            listContext.type === "ul" ? "• " : `${listContext.index}. `;
          segments.push({
            type: "text",
            text: bullet,
            color: newColor,
        ...
        node.childNodes.forEach(child => {
          if (child.nodeType === Node.TEXT_NODE) {
            ...
            traverse(
              child,
              { color: newColor, isBold: newBold, isUnderline: newUnderline },
              newListContext
            );
          }
        });
    ...
    traverse(
      root,
      { color: "#000000", isBold: false, isUnderline: false },
      null
    );
    ...
    segments.forEach(seg => {
      ...
      if (seg.type === "text") {
        ...
        words.forEach(word => {
          if (word === "") return;
          ctx.font = `${
            seg.isBold ? "bold" : "normal"
          } ${CURRENT_FONT_SIZE}px ${CURRENT_FONT.family}`;
          const wordWidth = measureTextWithSpacing(
            word,
            ctx.font,
            spacingFactor
          );
  ...
  useEffect(() => {
    pages.forEach((pageData, index) => {
      ...
      const rng = mulberry32(
        index + baseFontSize + Math.floor(skewFactor * 100)
      );
      ...
      pageData.lines.forEach(line => {
        ...
        if (dominantAlign === "center")
          cursorX = MARGIN_X + (availableWidth - lineTextWidth) / 2;
        else if (dominantAlign === "right")
          cursorX = MARGIN_X + (availableWidth - lineTextWidth);
            ...
            rows.forEach((row, rIdx) => {
              ...
              row.cells.forEach((cell, cIdx) => {
                ...
                if (cell) {
                  ...
                  ctx.lineTo(tableX + fullWidth, tableY);
                  ctx.moveTo(tableX, tableY + fullHeight);
                  ctx.lineTo(tableX + fullWidth, tableY + fullHeight);
                  ctx.moveTo(tableX, tableY);
                  ctx.lineTo(tableX, tableY + fullHeight);
                  ctx.moveTo(tableX + fullWidth, tableY);
                  ctx.lineTo(tableX + fullWidth, tableY + fullHeight);
                  ctx.stroke();

                  ctx.font = `${
                    cell.isBold ? "bold" : "normal"
                  } ${CURRENT_FONT_SIZE}px ${CURRENT_FONT.family}`;
                  ...
                  for (const char of cell.text)
                    textWidth +=
                      ctx.measureText(char).width + spacingFactor * SCALE;

                  let textX = tableX + 10 * SCALE;
                  if (cell.align === "center")
                    textX = tableX + fullWidth / 2 - textWidth / 2;
                  if (cell.align === "right")
                    textX = tableX + fullWidth - textWidth - 10 * SCALE;
  ...
  const saveProject = () => {
    ...
    // Option A: Specific to your app name (Recommended)
    a.download = "assignment.lazy";

    // Option B: Generic JSON
  ...
  return (
    <div
      className={`min-h-screen flex flex-col items-center font-sans transition-colors duration-300 ${
        isDarkMode
    ...
    >
      ...
      <header
        className={`sticky top-0 z-50 w-full px-6 py-4 transition-colors duration-300 ${
          isDarkMode
            ? "bg-slate-900/90 border-slate-700"
            : "bg-white/90 border-slate-200"
        } backdrop-blur-md border-b flex justify-between items-center shadow-sm`}
      >
          ...
          <div className="flex gap-3">
            ...
            <div className="flex gap-2">
              ...
              <input
                ...
                // CHANGE ACCEPTED EXTENSION HERE
                accept=".lazy,.json" // Accepts new .lazy files, generic .json, and files
                className="hidden"
              />
              ...
              <button
                onClick={downloadPDF}
                className="bg-blue-600 hover:bg-blue-700 text-white px-5 py-2 rounded-lg shadow-md text-sm font-medium flex gap-2 items-center transition-transform hover:scale-105"
              >
                <Download size={16} />
      ...
      <div className="w-full max-w-6xl flex flex-col lg:flex-row gap-8 py-8 lg:h-[calc(100vh-80px)] h-auto overflow-y-auto lg:overflow-hidden">
        {/* LEFT PANEL */}
        <div
          className={`w-full lg:w-1/2 flex flex-col rounded-xl shadow-xl border overflow-hidden transition-colors ${
            isDarkMode
        ...
        >
          ...
          >
            <div className="flex flex-wrap gap-2 justify-between items-center">
              <div className="flex gap-2 items-center">
                ...
                <button
                  ...
                  className={`p-2 rounded hover:bg-opacity-20 hover:bg-slate-500 ${
              ...
              <div className="flex gap-2 items-center">
                ...
                <div className="relative case-selector">
                  <button
                    ...
                    className={`flex items-center gap-1 px-3 py-1.5 rounded border text-sm font-medium ${
                      isDarkMode
                  ...
                  {isCaseMenuOpen && (
                    <div className="absolute top-full left-0 mt-2 w-32 bg-white border rounded-lg shadow-xl z-50 overflow-hidden py-1">
                      <button
                        ...
                        className="w-full text-left px-4 py-2 text-sm hover:bg-blue-50 text-black flex items-center gap-2"
                      >
                        <CaseUpper size={14} /> UPPER
                      </button>
                      <button
                        onClick={() => changeCase("lower")}
                        className="w-full text-left px-4 py-2 text-sm hover:bg-blue-50 text-black flex items-center gap-2"
                      >
                        <CaseLower size={14} /> lower
                      </button>
                      <button
                        onClick={() => changeCase("title")}
                        className="w-full text-left px-4 py-2 text-sm hover:bg-blue-50 text-black flex items-center gap-2"
                      >
                        <CaseSensitive size={14} /> Title
                      </button>
                    </div>
                  )}
                </div>
                ...
                <button
                  ...
                  className={`p-2 rounded border hover:bg-red-100 text-red-500 ${
                    isDarkMode
                      ? "border-slate-600 hover:bg-red-900"
                      : "border-slate-200"
                  } ${selectedImage ? "ring-2 ring-red-400" : ""}`}
                  title="Delete Selected Image"
                >
                  <ImageMinus size={18} />
                </button>
                <label
                  className={`p-2 rounded border cursor-pointer hover:bg-opacity-10 ${
                    isDarkMode
                      ? "border-slate-600 text-slate-300 hover:bg-white"
                      : "border-slate-200 text-slate-700 hover:bg-black"
                  }`}
                  title="Insert Image"
                >
                  <ImageIcon size={18} />
            ...
            <div className="flex items-center gap-2 p-2 rounded-lg bg-blue-50/50 border border-blue-100 flex-wrap">
              ...
              <button
                onClick={insertTable}
                className="p-1.5 rounded hover:bg-blue-100 text-blue-800"
                title="New Table"
              >
                <TableIcon size={16} />
              </button>
              ...
              <button
                ...
                className="p-1.5 rounded hover:bg-blue-100 text-blue-800 flex gap-1 text-xs items-center"
                title="Add Row"
              >
                <Rows size={14} />
                <Plus size={10} />
              ...
              <button
                onClick={() => modifyTable("delRow")}
                className="p-1.5 rounded hover:bg-red-100 text-red-600 flex gap-1 text-xs items-center"
                title="Delete Row"
              >
                <Rows size={14} />
                <Trash2 size={10} />
              ...
              <div className="w-px h-4 bg-blue-200"></div>
              <button
                onClick={() => modifyTable("mergeRight")}
                className="p-1.5 rounded hover:bg-purple-100 text-purple-800 flex gap-1 text-xs items-center"
                title="Merge Right"
              >
                <ArrowRightFromLine size={14} />
              </button>
              <button
                onClick={() => modifyTable("mergeDown")}
                className="p-1.5 rounded hover:bg-purple-100 text-purple-800 flex gap-1 text-xs items-center"
                title="Merge Down"
              >
            ...
            <div className="grid grid-cols-2 gap-6 pt-2">
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-xs font-semibold uppercase tracking-wider opacity-70">
                  <span>Messiness</span> <span>{skewFactor}</span>
                </div>
                <input
                  type="range"
                  min="0"
                ...
                <div className="flex justify-between text-xs font-semibold uppercase tracking-wider opacity-70 mt-1">
                  <span>Font Size</span> <span>{baseFontSize}px</span>
                </div>
                <input
                  type="range"
                  min="14"
              ...
              <div className="space-y-3">
                <div className="flex justify-between text-xs font-semibold uppercase tracking-wider opacity-70">
                  <span>Letter Spacing</span> <span>{spacingFactor}px</span>
                </div>
                <input
                  type="range"
                  min="-2"
                ...
                <div className="flex gap-2 pt-1">
                  <button
                    ...
                    className={`flex-1 py-1.5 rounded text-xs font-medium border flex items-center justify-center gap-2 transition-colors ${
                      isDarkMode
                  ...
                  <button
                    onClick={() => setScanEffect(!scanEffect)}
                    className={`flex-1 py-1.5 rounded text-xs font-medium border flex items-center justify-center gap-2 transition-colors ${
                      scanEffect
                  ...
                  >
                    <ScanLine size={14} />{" "}
                    {scanEffect ? "Scanner ON" : "Scanner OFF"}
                  </button>
                </div>
              </div>
            </div>
          ...
          <div
            className="flex-1 overflow-y-auto bg-white text-slate-900 relative"
            onClick={handleEditorClick}
          >
            <div
              ref={editorRef}
              contentEditable
              onInput={triggerParse}
              className="min-h-full p-8 outline-none font-sans text-lg leading-relaxed"
            ></div>
          </div>
        </div>
        ...
        {/* RIGHT PREVIEW */}
        <div
          className={`w-full lg:w-1/2 overflow-y-auto rounded-xl shadow-inner border p-8 flex flex-col items-center gap-8 ${
            isDarkMode
        ...
        >
          {pages.map((_, i) => (
            <div key={i} className="relative group shrink-0 max-w-full">
              <span
                className={`absolute -left-10 top-0 font-bold text-xs opacity-50 ${
              ...
              <canvas
                ...
                className="bg-white shadow-2xl rounded-sm transition-transform duration-300 hover:scale-[1.02] max-w-full h-auto"
              />
            </div>
          ))}
        </div>
